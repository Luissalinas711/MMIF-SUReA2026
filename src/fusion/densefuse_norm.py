...
def norm_fuse_features(features_A, features_B, window=3):
    # Forhe norm strategy instead of averaging the two feature maps equally, weight each source by how "active" it is at each spot, so the stronger source wins there.
    # Activity map = sum of the absolute feature values across all channels
    activity_A = torch.sum(torch.abs(features_A), dim=1, keepdim=True)
    activity_B = torch.sum(torch.abs(features_B), dim=1, keepdim=True)

    # Smooth each activity map with a (small) local average
    pad = window // 2
    box = torch.ones(1, 1, window, window, device=device) / (window * window)
    activity_A = F.conv2d(activity_A, box, padding=pad)
    activity_B = F.conv2d(activity_B, box, padding=pad)

    # Turn the activities into weights that add up to 1 at each spot
    total = activity_A + activity_B + 1e-10
    weight_A = activity_A / total
    weight_B = activity_B / total

    # Combine the feature maps using those weights
...
def build_fuser(weights_path):
    def fuse(image_A, image_B):
        ...
        with torch.no_grad():
            ...
            # Fuse the feature maps using the norm strategy 
            # The encoder returns a list, so we pass the tensor inside it (index 0)
            fused = norm_fuse_features(features_A[0], features_B[0])

            # Decode the fused feature map back into an image
            # The decoder takes a list, so we wrap the fused tensor back in one
            fused_output = model.decoder([fused])[0]
